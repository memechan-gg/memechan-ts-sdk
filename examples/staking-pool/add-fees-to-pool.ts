...
export const addFeesToPool = async () => {
  ...
  const addFeesToStakingPoolTxTxResult = await provider.signAndExecuteTransactionBlock({
    transactionBlock: addFeesToStakingPoolTx.tx,
    signer: keypair,
    options: {
  ...
  console.debug("addFeesToStakingPoolTxTxResult: ", addFeesToStakingPoolTxTxResult);
};
